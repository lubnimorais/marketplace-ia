  ...
  implements OnApplicationBootstrap, OnApplicationShutdown
{
  ...
  async onApplicationBootstrap() {
    return this.client
      ...
      .catch((err) => this.logger.error('Error connecting to Postgres', err));
  }

  async onApplicationShutdown() {
    return this.client
      .end()
      .then(() => this.logger.log('Disconnected from Postgres'))
      .catch((err) =>
        this.logger.error('Error disconnecting from Postgres', err),
      );
  }
}
