...
export class GeminiLlmService extends LlmService {
  ...
  async batchEmbedProducts(
    ...
    const result = await this.client.models.embedContent({
      contents: products.map((product) => product.name),
    ...
    if (!result.embeddings || result.embeddings.length === 0) {
      console.error('No embeddings returned for batch embedding');
      return;
    }
    ...
    for (const [index, resultItem] of result.embeddings.entries()) {
      if (!resultItem.values || resultItem.values.length === 0) {
        console.error(
          `No embedding returned for product ${products[index].name}`,
        );
        continue;
      }
