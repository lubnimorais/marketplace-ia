...
export class CatalogService implements OnApplicationBootstrap {
  ...
  //Quando a aplicação form inicializada, esse serviço vai chamar essa função, que vai interagir com o LlmService
  async onApplicationBootstrap() {
    if (this.configService.get<string>('NODE_ENV') === 'test') {
      return;
    }

    const products = await this.postgresService.client.query<IProduct>(
